...
async def google_start():
    ...
    <body>
        ...
        <script>
            window.location.href = "{GOOGLE_AUTH_URL}?response_type=code&client_id={GOOGLE_CLIENT_ID}&redirect_uri={GOOGLE_REDIRECT_URI}&scope=openid%20email%20profile&access_type=offline&prompt=consent";
        </script>
    </body>
    </html>
    """
...
async def google_callback(request: Request, db: AsyncSession = Depends(get_db)):
    try:
        ...
        if not email:
            raise HTTPException(status_code=400, detail="Failed to extract email")
